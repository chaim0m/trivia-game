...
export default class Game extends Component {
    ...
    getQuestionsApi(category, difficulty){
        ...
        .then(response => {
            console.log(response)
                ...
                this.setState({questions: [questions], userAnswers: []});
        })
        .catch(error => {
    ...
    render(){
        ...
        let questionBox = (this.state.questions) ? (<QuestionBox item={this.showQuestion()} handleAnswer={this.handleAnswer}/>):(<p>LOADING</p>);
        return (
            <div>
                {questionBox} 
            </div>
